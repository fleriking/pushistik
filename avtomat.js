...
function addCoins(n){
 ...
 let col1234=[{name: '1',src:'1.jpg', chance: 0.377, position: 0},
...
const mass_win = [table_win.earn_1,table_win.earn_2,table_win.earn_3,table_win.earn_smile]
...
function onClick(event){
    ...
    if (event.target.id === 'stavka') {
    	...
    	if(whithoutWild.length==0)
    	{
    	...
    	}else if(whithoutWild.length==1 &&whithoutWild[0].name=='x2') { earn_avtomat = table_win.JACK_POT_x2 }
    	else if(whithoutWild.length==1 &&whithoutWild[0].name=='x4') { earn_avtomat = table_win.JACK_POT_x4 }
    	else {
    		// nija mistakes
    		if (whithoutWild.length==1) {
    			...
    			if(whithoutWild[0].name!='x2'&& whithoutWild[0].name!='x4'){
    				...
    				earn_avtomat=+(whithoutWild[0].name[1])*table_win[`earn_${whithoutWild[1].name}`]
    	...
    	earn_avtomat ? messages.textContent=`You are win: ${Number((earn_avtomat*multy).toFixed(2))} coins` : messages.textContent='You are looser'
...
function onClickPlusMany(event){
	...
	stavka_plus.addEventListener("mouseup", function(){
	 	clearInterval(x)
	 })
}
function onClickMinusMany(event){
	...
	//}
	 ...
	 stavka_minus.addEventListener("mouseup", function(){
	 	clearInterval(x)
	 })
...
function getchance(col0,col1234)
{
	...
	console.log('jackpot chance:',jackpot,'  chance*win=',jackpot*table_win.JACK_POT)
	...
	console.log('jackpot_x4 chance:',jackpotx4,'  chance*win=',jackpotx4*table_win.JACK_POT_x4)

	chance_win+=jackpotx4
	rake+=jackpotx4*table_win.JACK_POT_x4

	let jackpotx2 = dif0[5]*dif1234[4]*dif1234[4]*dif1234[4]*dif1234[4]
	console.log('jackpot_x2 chance:',jackpotx2,'  chance*win=',jackpotx2*table_win.JACK_POT_x2)
	...
	for (let i=0;i<4;i++){
		...
		console.log(i,' chance0=',chanceCols0[i],'  chance*win=',chanceCols0[i]*mass_win[i])
	...
	for (let i=0;i<4;i++){
		...
		console.log(i,' chance0x2=',chanceColsx2[i],'  chance*win=',chanceColsx2[i]*mass_win[i]*2)
	...
	for (let i=0;i<4;i++){
		...
		console.log(i,' chance0x4=',chanceColsx4[i],'  chance*win=',chanceColsx4[i]*mass_win[i]*4)
